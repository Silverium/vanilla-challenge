...
export const dataDigester = () => {
   ...
   */
  const hostAppEntryDigester = (rankedAppsByHost : RankedAppsByHost) => (appData: AppData) => {
   ...
   * Complexity is O(n2) as we only loop on each host, and then on each element of the ranking.
   */
  const hostAppEntryGarbager = (rankedAppsByHost: RankedAppsByHost) => (AppData: AppData) => {
    const { host = [] } = AppData;
    host.forEach(
      elementInSortedDictionaryRemover(rankedAppsByHost, AppData)
    );
  };
   ...
   */
  const digestHostAppData = (list: AppData[] = []): RankedAppsByHost => {
